...
def highlight_special(dataframe: pd.DataFrame):
    ...
    # 2列目（男性和）の最大値を赤
    max_men_sum = dataframe["男性和"].max()
    styles.loc[dataframe["男性和"] == max_men_sum, "男性和"] = "color: red; font-weight: bold;"

    # 4列目（差）の最小値を赤
    min_diff = dataframe["差"].min()
    styles.loc[dataframe["差"] == min_diff, "差"] = "color: red; font-weight: bold;"

    # 5列目（最小）の最大値を赤
    max_min_value = dataframe["最小"].max()
    styles.loc[dataframe["最小"] == max_min_value, "最小"] = "color: red; font-weight: bold;"
